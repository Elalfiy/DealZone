...
const MyRFQs = () => {
  const { user, requests, offers } = useApp()
  const { t, language } = useTranslation()
  const myRFQs = requests.filter(r => r.manufacturerId === user?.id) || []
      ...
      <div className={`ml-0 ${language === 'ar' ? 'md:mr-64' : 'md:ml-64'} pt-20 p-8`}>
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-4xl font-cairo font-bold text-text-dark">
              {t('rfq.myRFQs')}
            </h1>
            <Link to="/create-rfq">
          ...
          {myRFQs.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {myRFQs.map((rfq) => {
                const offerCount = offers.filter(o => o.requestId === rfq.id).length
                return (
